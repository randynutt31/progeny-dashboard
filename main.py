...
<style>
  ...
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  .progress-fill {{ background: #c9a84c; border-radius: 3px; height: 4px; transition: width 0.3s; }}
  ...
  .niche-name {{ font-size: 14px; font-weight: 600; margin-bottom: 4px; }}
  .niche-score {{ font-size: 28px; font-weight: 700; color: #c9a84c; }}
  .niche-verdict {{ font-size: 11px; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 10px; }}
...
<div class="topbar">
  ...
  <div class="agent-status">
    ...
    <span id="agentStatus">Agent connecting...</span>
  </div>
</div>
...
<div class="panel active" id="panel-command">
  <div class="projects-grid">
    ...
    <div class="project-card">
      ...
      <div class="project-desc">Brain is running on Railway. Control panel connected. Self-improving.</div>
...
<div class="panel" id="panel-tracker">
  <div class="card">
    ...
    <div class="tracker-row tracker-header">
      <div>Project</div><div>Status</div><div>Progress</div><div>Next Action</div>
    </div>
    <div class="tracker-row">
      <div style="font-weight:600">ProgenyVault</div>
      <div><span class="badge priority">Priority</span></div>
      <div style="color:#c9a84c">45%</div>
      <div style="color:#888;font-size:12px">Stack orientation session → resume build</div>
    </div>
    <div class="tracker-row">
      <div style="font-weight:600">Den of Indigos</div>
      <div><span class="badge active">Active</span></div>
      <div style="color:#4caf50">60%</div>
      <div style="color:#888;font-size:12px">Resend domain verify → email sequences live</div>
    </div>
    <div class="tracker-row">
      <div style="font-weight:600">ReptiTerra Labs</div>
      <div><span class="badge backburner">Backburner</span></div>
      <div style="color:#555">70%</div>
      <div style="color:#888;font-size:12px">Write Tropical + Tunnel Blend copy</div>
    </div>
    <div class="tracker-row">
      <div style="font-weight:600">Vault Trader</div>
      <div><span class="badge blocked">Blocked</span></div>
      <div style="color:#e05555">90%</div>
      <div style="color:#888;font-size:12px">Authy 2FA → Alpaca keys → Railway deploy</div>
    </div>
    <div class="tracker-row">
      <div style="font-weight:600">Progyny Infinite Trust</div>
      <div><span class="badge blocked">Pending</span></div>
      <div style="color:#e05555">20%</div>
      <div style="color:#888;font-size:12px">Wait for DOR audit letter June 30</div>
    </div>
    <div class="tracker-row">
      <div style="font-weight:600">PICP + Agent</div>
      <div><span class="badge active">Live</span></div>
      <div style="color:#4caf50">85%</div>
      <div style="color:#888;font-size:12px">Package for Shawn Dark + user manual</div>
    </div>
  </div>

  <div class="card">
    <div class="card-title">Factory Pipeline — Next Builds</div>
    <div class="tracker-row tracker-header">
      <div>Niche</div><div>Score</div><div>Status</div><div>Notes</div>
    </div>
    <div class="tracker-row">
      <div>Koi / Fancy Goldfish</div><div style="color:#c9a84c;font-weight:700">42</div><div><span class="badge active">GO</span></div><div style="font-size:12px;color:#666">Highest score</div>
    </div>
    <div class="tracker-row">
      <div>Tarantula / Invert</div><div style="color:#c9a84c;font-weight:700">41</div><div><span class="badge active">GO</span></div><div style="font-size:12px;color:#666">Huge community</div>
    </div>
    <div class="tracker-row">
      <div>Dart Frog</div><div style="color:#c9a84c;font-weight:700">40</div><div><span class="badge active">GO</span></div><div style="font-size:12px;color:#666">Complex genetics</div>
    </div>
    <div class="tracker-row">
      <div>Small Livestock</div><div style="color:#c9a84c;font-weight:700">38</div><div><span class="badge active">GO</span></div><div style="font-size:12px;color:#666">Fastest build — PV reuse</div>
    </div>
    <div class="tracker-row">
      <div>Entertainer Analytics</div><div style="color:#555">TBD</div><div><span class="badge backburner">Queued</span></div><div style="font-size:12px;color:#666">$14.99/mo, stigma moat</div>
    </div>
    <div class="tracker-row">
      <div>Philately</div><div style="color:#555">TBD</div><div><span class="badge backburner">Queued</span></div><div style="font-size:12px;color:#666">$19.99/mo, $3.4B market</div>
    </div>
  </div>
</div>
  ...
  <div class="two-col">
    <div>
      <div class="card">
        ...
        <div class="chip-row">
          <button class="chip" onclick="setSource('session')">Session Extract</button>
          <button class="chip" onclick="setSource('decision')">Decision Log</button>
          <button class="chip" onclick="setSource('project')">Project Update</button>
        </div>
    ...
    <div>
      <div class="card">
        ...
        <div class="chip-row">
          <button class="chip" onclick="setQuery('What is the current status of ProgenyVault?')">PV Status</button>
          <button class="chip" onclick="setQuery('What are all open flags right now?')">Open Flags</button>
          <button class="chip" onclick="setQuery('What is the next niche to build?')">Next Niche</button>
        </div>
...
<div class="panel" id="panel-niche">
  <div class="card">
    ...
    <button class="btn" onclick="scoreNiche()" style="margin-top:12px;">Score It</button>
    <div class="result" id="nicheResult"></div>
  </div>
  <div class="card">
    <div class="card-title">Current Pipeline</div>
    <div class="score-grid" id="nicheGrid">
      ...
      <div class="niche-card"><div class="niche-name">Dart Frog Breeding</div><div class="niche-score">40</div><div class="niche-verdict go">GO</div></div>
...
<div class="panel" id="panel-youtube">
  <div class="card">
    ...
    <div class="chip-row" style="margin-top:12px;">
      <button class="chip" onclick="setYtMode('wiki')">Wiki Seed</button>
      <button class="chip" onclick="setYtMode('build')">Build Extract</button>
      <button class="chip" onclick="setYtMode('summary')">Quick Summary</button>
      <button class="chip" onclick="setYtMode('picp')">PICP Intel</button>
    </div>
...
<div class="panel" id="panel-finance">
  ...
  <div class="card">
    ...
    <div style="font-size:13px;color:#555;line-height:1.7;">
      ...
      <div>Universe: 50 large-cap symbols</div>
      <div>Position size: 15% | Stop: 3% | Take profit: 5%</div>
...
<div class="panel" id="panel-tools">
  <div class="tools-grid">
    <div class="tool-card" onclick="switchTab('niche')">
      <div class="tool-icon">🎯</div>
      <div class="tool-name">Niche Scorer</div>
      <div class="tool-desc">Score any niche against the 5-criteria Factory formula</div>
    </div>
    <div class="tool-card" onclick="switchTab('youtube')">
      <div class="tool-icon">▶️</div>
      <div class="tool-name">YouTube Extractor</div>
      <div class="tool-desc">Extract concepts from any YouTube video automatically</div>
    </div>
    <div class="tool-card" onclick="switchTab('finance')">
      <div class="tool-icon">📈</div>
      <div class="tool-name">Finance AI</div>
      <div class="tool-desc">Market queries, stock lookup, Vault Trader status</div>
    </div>
    <div class="tool-card" onclick="switchTab('agent')">
      <div class="tool-icon">🧠</div>
      <div class="tool-name">Brain Control</div>
      <div class="tool-desc">Send context to the PICP agent, query the brain</div>
    </div>
    <div class="tool-card" onclick="window.open('https://claude.ai','_blank')">
      <div class="tool-icon">⚡</div>
      <div class="tool-name">Open Claude</div>
      <div class="tool-desc">Launch Claude in a new tab for working sessions</div>
    </div>
    <div class="tool-card" onclick="window.open('https://github.com/randynutt31','_blank')">
      <div class="tool-icon">🐙</div>
      <div class="tool-name">GitHub</div>
      <div class="tool-desc">View and manage your repos</div>
    </div>
    <div class="tool-card" onclick="window.open('https://railway.app','_blank')">
      <div class="tool-icon">🚂</div>
      <div class="tool-name">Railway</div>
      <div class="tool-desc">Monitor deployed services and logs</div>
    </div>
    <div class="tool-card" onclick="window.open('https://supabase.com','_blank')">
      <div class="tool-icon">🗄️</div>
      <div class="tool-name">Supabase</div>
      <div class="tool-desc">Database management for all products</div>
    </div>
  </div>
</div>
...
async function checkAgent() {{
  try {{
    ...
    document.getElementById('agentStatus').textContent = 'Brain online';
  }} catch(e) {{
    document.getElementById('agentDot').className = 'status-dot';
    document.getElementById('agentStatus').textContent = 'Brain offline';
  }}
}}
checkAgent();
...
// AGENT CONTROL
function setSource(s) {{ document.getElementById('sourceInput').value = s; }}
...
async function ingestContext() {{
  ...
  try {{
    const res = await fetch(AGENT + '/ingest', {{
      method: 'POST',
  ...
  try {{
    const res = await fetch('https://api.anthropic.com/v1/messages', {{
      ...
      body: JSON.stringify({{
        ...
        max_tokens: 1000,
        messages: [{{ role: 'user', content: `Score this niche for the Progyny Infinite Factory using these 5 criteria (0-10 each, max 50):
1. Community exists (active forums, groups, associations)
2. Ops problem exists (spreadsheets/paper/nothing)
3. No dominant tool (nothing good on Google)
4. Willingness to pay (money in the niche)
5. Too small for big players (under VC radar)

GO threshold: 35+/50. ProgenyVault benchmarks 42-45.

Niche: ${{niche}}

Give score per criterion, total score, GO/MAYBE/NO-GO verdict, top opportunity, biggest risk. Be blunt and direct.` }}]
      }})
    }});
    const data = await res.json();
    result.className = 'result visible success';
...
// YOUTUBE EXTRACTOR
function setYtMode(m) {{ document.getElementById('ytMode').value = m; }}
...
async function extractYoutube() {{
  ...
  result.className = 'result visible';
  const prompts = {{
    wiki: `Extract key concepts from this YouTube video for Randy's Progyny Infinite business wiki. URL: ${{url}}. Give: core concepts, actionable techniques, tools mentioned, direct applicability to PICP/Factory build, what to steal.`,
    build: `Extract everything buildable from this YouTube video. URL: ${{url}}. Give: exact file structure, code patterns, build order, what to skip.`,
    summary: `Summarize this YouTube video in under 200 words for Randy, a solo entrepreneur building a micro-SaaS factory. URL: ${{url}}. Give: one sentence what it is, 3 key points, verdict: worth building from?`,
    picp: `What in this YouTube video applies to Randy's PICP build (Claude Project + agent + dashboard + wiki)? URL: ${{url}}. Give: structure ideas, file patterns, automation ideas, prompts to steal.`
  }};
  try {{
    const res = await fetch('https://api.anthropic.com/v1/messages', {{
      method: 'POST',
...
async function queryFinance() {{
  ...
  result.textContent = '⬤ Searching markets...';
  result.className = 'result visible';
  try {{
    const res = await fetch('https://api.anthropic.com/v1/messages', {{
      method: 'POST',
